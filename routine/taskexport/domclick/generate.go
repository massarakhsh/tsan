...
//	Подготовка отчета
func (it *TaskDomclick) PrepareReport() bool {
	result := false
	if it.Set != nil {
...
//	Запись файла
func (it *TaskDomclick) WriteFiles() {
	it.WriteFileXML(it.Report)
}

//	Добавление в отчёт Авито заявки
//	elm - добавляемая заявка
func (it *TaskDomclick) AppendOffer(elm *likbase.ItElm) string {
...
func (it *TaskDomclick) appendOfferMain(elm *likbase.ItElm, content lik.Lister) string {
	return ""
}

//	Сведения о продавце
func (it *TaskDomclick) appendOfferSaler(elm *likbase.ItElm, content lik.Lister) string {
...
func (it *TaskDomclick) appendOfferAddress(elm *likbase.ItElm, content lik.Lister) string {
	if text := it.MakeAddress(elm); text != "" {
		it.SetValue(content, "Address", text)
	} else {
		return "Отсутсвует адрес объекта"
	}
	if info := jone.CalculateElmString(elm, "objectid/address/apartment"); info != "" {
		it.SetValue(content, "FlatNumber", info)
	}
	return ""
}

//	Описание объекта
func (it *TaskDomclick) appendOfferDefine(elm *likbase.ItElm, content lik.Lister) string {
...
//	Параметры объекта
func (it *TaskDomclick) appendOfferParams(elm *likbase.ItElm, content lik.Lister) string {
...
func (it *TaskDomclick) appendOfferPhotos(elm *likbase.ItElm, content lik.Lister) string {
